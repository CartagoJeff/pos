        ...
        if (isset($_POST['job_type'])) {
            ...
            if ($jobOrder) {
                foreach ($_POST['job_type'] as $index => $type) {
                    ...
                    $quantity = ($type === 'part') ? intval($_POST['job_quantity'][$index]) : floatval($_POST['job_quantity'][$index]);
                        ...
                        <div class="job-description-block">
                            ...
                            <div class="form-row">
                                <div class="form-group" style="flex: 1;">
                                    ...
                                    <input type="number" class="job-qty" name="job_quantity[]" value="<?php echo ($job['type'] === 'part') ? intval($job['quantity']) : htmlspecialchars($job['quantity']); ?>" min="1" step="<?php echo ($job['type'] === 'part') ? '1' : '0.01'; ?>" onchange="validateQuantity(this)">
                                </div>
                                <div class="form-group" style="flex: 1;">
                                    <label>Unit</label>
    ...
    <script>
        ...
        function updateDescriptionField(select) {
            ...
            if (select.value === 'part') {
                ...
                qtyInput.step = '1'; // Whole numbers for parts
                qtyInput.value = Math.round(qtyInput.value); // Round to nearest whole number
            } else {
                partSelectContainer.style.display = 'none';
                descriptionTextarea.style.display = 'block';
                descriptionTextarea.required = true;
                container.querySelector('.part-select').required = false;
                qtyInput.step = '0.01'; // Decimals allowed for labor
            }
        }
